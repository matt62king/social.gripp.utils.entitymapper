...
import com.greenfrog.utils.datastore.mapper.mapper.properties.PropertyGenerator;
...
public class EntityMapper<BEAN>  {
    ...
    private final Map<String, Value<?>> propertyMap = new HashMap<>();
    private final Datastore datastore = new DatastoreOptions.DefaultDatastoreFactory().create(DatastoreOptions.getDefaultInstance());
    private final PropertyGenerator propertyGenerator = new PropertyGenerator(propertyMap);
    ...
    private void tryToSetEntityProperty(Field field, final BEAN bean) {
        ...
        try {
            if (AnnotationUtils.isAutoGenerated(field) && field.get(bean) == null) {
                propertyGenerator.generateProperty(field);
            }
            else {
                writePropertyFromBean(field, bean);
            }
        }
        catch (IllegalAccessException ex) {
            throw new FieldAccessException(field, bean.getClass());
    ...
    private void writePropertyFromBean(Field field, final BEAN bean) throws IllegalAccessException {
        setPropertyIfPresent(
                AnnotationUtils.getColumnValue(field),
                field.get(bean),
